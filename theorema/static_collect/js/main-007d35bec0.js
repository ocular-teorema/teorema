...
var module = angular.module('app', ['ui.router', 'Constants', 'Services', 'Directives', 'Filters']);

module.controller('baseController', function($scope) {

}).controller('mainMenuController', function($scope, Menu) {
    $scope.mainMenu = Menu;
...
angular
    ...
    .controller('recordsController',
        function($scope, RecordsService, $filter, $rootScope, EventsDescription, $timeout) {
            ...
            $scope.endTime = statParams['endTime'] || '23:30';

            // $scope.$watch('startTime', function() {
            //     var startTime = $scope.startTime.split(':');
            //     var startMinutes = startTime[0] * 60 + startTime[1] * 1;
            //     var endTime = $scope.endTime.split(':');
            //     var endMinutes = endTime[0] * 60 + endTime[1] * 1;
            //     if (startMinutes >= endMinutes) {
            //         var endChangedMinuts = Math.min();
            //     }
            // });
            ...
            // $scope.selectedCamera = angular.copy($scope.cameraModel);
            ...
            var parseStatistics = function(data) {
                $scope.eventsList = [];
                $scope.selectedCamera = $scope.cameraModel;
                data.map(function(eventItem) {
                    var date = angular.element.datepicker.parseDate("yy-mm-dd", eventItem.date);
                    var currentEvent;
                ...
                $scope.eventsList.map(function(eventOneDay) {
                    eventOneDay.events.map(function(ev) {
                        ev.archivePostfix = ev.archivePostfix.replace(/[\d]{2}(\.[\S]{3}$)/, '00$1');
                    });
                });
                $scope.applyFilters();
            ...
            var videoLengthMinuts = 10;

            var generateStatistic = function() {
                var splittedStartTime = $scope.startTime.split(':');
                var startMinuts = splittedStartTime[0] * 60 + splittedStartTime[1] * 1;
                var splittedEndTime = $scope.endTime.split(':');
                var endMinuts = splittedEndTime[0] * 60 + splittedEndTime[1] * 1;
                var dayPeriod = 3600000 * 24;

                var startDate = angular.element.datepicker.parseDate("dd.mm.yy D", $scope.startDate);
                var endDate = angular.element.datepicker.parseDate("dd.mm.yy D", $scope.endDate);

                var daysCount = Math.ceil((endDate.getTime() - startDate.getTime()) / dayPeriod);

                var periodsCount = endMinuts - startMinuts;
                var statArray = [];

                var camName = $scope.cameraModel ? 'cam' + $scope.cameraModel.id : '';
                for (var k = 0; k <= daysCount; k++) {
                    var date = new Date(startDate.getTime() + dayPeriod * k);
                    for (var t = 0; t <= periodsCount; t+=videoLengthMinuts) {
                        var timeMinutes = startMinuts + t;
                        date.setHours(Math.floor(timeMinutes / 60));
                        date.setMinutes(timeMinutes % 60);
                        date.setSeconds(0);

                        statArray.push({
                            archivePostfix: '/' + camName + '/' + camName + '_' + $filter('date')(date, 'dd_MM_yyyy___HH_mm_ss') + '.mp4',
                            start: $filter('date')(date, 'HH-mm'),
                            end: $filter('date')(new Date(date.getTime() + videoLengthMinuts * 600000), 'HH-mm'),
                            date: $filter('date')(date, 'yyyy-MM-dd'),
                            events: []
                        });
                    }
                }
                parseStatistics(statArray);
                $scope.selectedCamera = $scope.cameraModel;
            };
            ...
            $scope.getStatistic = function() {
                ...
                // if ($scope.cameraModel && ($scope.cameraModel.analysis != 1)) {
                    $scope.sendForm = true;
                ...
                // } else if ($scope.cameraModel) {
                //     $timeout(generateStatistic);
                // }
            };
            $scope.getStatistic();
...
angular
    ...
    .controller('recordsPlayerController',
        function($q, $scope, $rootScope, $filter, $state, RecordsService, $timeout, MonitoringService, EventsDescription) {
            ...
            var modesParams = {0: 1, 1: 2, 2: 4, 3: 16, 4: 25};
...
var module = angular.module('app');
module.controller('streamController', function($scope, currentUser, $rootScope, $state, Windows, CollectionsService, camerasCollections, $interval, MonitoringService, $timeout) {
    ...
    var toggleFullScreen = function(open) {
        ...
        if (open && ((document.fullScreenElement !== undefined && document.fullScreenElement === null) || (document.msFullscreenElement !== undefined && document.msFullscreenElement === null) || (document.mozFullScreen !== undefined && !document.mozFullScreen) || (document.webkitIsFullScreen !== undefined && !document.webkitIsFullScreen))) {
    ...
    /* Определяем текущий квадратор */

    // $scope.camerasCollection = [];

    var localStorage = window.localStorage || {};
    var selectedCollection = localStorage ? localStorage['camCollection'] || false : false;


    $scope.streamMode = localStorage['streamMode'] ? JSON.parse(localStorage['streamMode']) : false;
    $scope.$watch('streamMode', function() {
        localStorage['streamMode'] = JSON.stringify($scope.streamMode);
    });


    $scope.camerasCollectionList = camerasCollections.data || [];
    if (selectedCollection) {
        var collectionInList = $scope.camerasCollectionList.filter(function(collection) {
            return collection.id == selectedCollection;
        })[0];
        if (collectionInList) {
            $scope.camerasCollection = collectionInList;
        }
    }

    var checkCamerasList = function() {
        if (!$scope.camerasCollectionList.length) {
            $scope.camerasCollectionList.unshift({
                name: 'default'
            });
        } else if (!$scope.camerasCollectionList[0]['id']) {
            $scope.camerasCollectionList.shift();
        }
    };
    checkCamerasList();
    ...
    /* Список всех доступных камер */
    var camerasList = [];
    angular.copy($rootScope.userCameras).groups.map(function(group) {
        var cameras = group.cameras;
        cameras.map(function(cam) {
            camerasList.push(cam);
        });
    ...
    $scope.setViewMode = function(mode) {
        ...
        hideCameraCategories();
        if ($scope.zoomedCamera) {
            if (oldShowedQuadrator.mode === mode) {
                $scope.hideCameraPreview();
                return;
            }
        }

        if ($scope.currentQuadrator.mode === mode) return;
        var noVisibleCameras = $scope.currentQuadrator.cameras;
        noVisibleCameras = noVisibleCameras.concat(angular.copy($scope.currentQuadrator.cameras).filter(function(cam) {
            return !noVisibleCameras.find(function(searchedCam) {
                return cam.id === searchedCam.id;
            });
        }));
        ...
        var newCamerasList = noVisibleCameras.slice(0, modesParams[mode]);

        $scope.currentQuadrator.mode = mode;
        $scope.currentQuadrator.cameras = angular.copy(newCamerasList);
    };

    /* Если нет сохраненных наборов, то создаем новый */
    // !$scope.currentQuadrator.id ? $scope.setViewMode(3) : false;

    /* Открываем диалоговое окно для ввода названия квадратора */
    var saveDialog = function(params, cb) {
        Windows.prompt({
            title: 'Сохранение набора',
            text: "Задайте имя для набора камер",
            field: {
                regexp: /[\S]+/,
                required: true,
                ngModel: params.name
            },
            onConfirm: function (field, onConfirm) {
    ...
    /* Управление выпадающими списками с категориями/камерами */
    var activeGroup = false;
    var showedCameraCategories = false;

    /* Открытие категории внутри списка */
    $scope.showCategoryInCatalog = function(group) {
        if (activeGroup) {
            activeGroup.active = false;
        }
    ...
    /* Анализ событий и вывод на экран */
    var analiseMonitoringEvents = function() {
        if (!$scope.currentQuadrator) return;
        if (!$scope.currentQuadrator.cameras) return;
        $scope.currentQuadrator.cameras.map(function(cam) {
            var lastVisibleEvent = cam.visibleEvent;
            var event = lastedEvents[cam.id] ? lastedEvents[cam.id].filter(function(ev) {
                return ev.confidence >= $scope.confidence;
            })[0] : false;
            var allVisibleEvents = lastedNoReactionEvents[cam.id] ? lastedNoReactionEvents[cam.id].filter(function(ev) {
                return ev.confidence >= $scope.confidence;
            }) : [];
            if (event) {
                cam.visibleEvent = event;
                if (event !== lastVisibleEvent) {
                    Beep.play();
                }
            } else {
                cam.visibleEvent = allVisibleEvents[0] || false;
            }
        });
    };

    $scope.$watch('confidence', analiseMonitoringEvents);

    /* Добавляем событие в список предыдущих событий камеры */
    var addEventForCamera = function(event, camera) {
        lastedEvents[camera.id] = lastedEvents[camera.id] || [];
        switch (event.type) {
            case 'start':
                event.info = MonitoringService.getEventInfo(event.eventType);
                lastedEvents[camera.id].unshift(event);
                break;
            case 'end':
                lastedEvents[camera.id] = lastedEvents[camera.id].filter(function(ev) {
                    return ev.id !== event.id;
                });
                if (event.reaction != -1) return;
                event.info = MonitoringService.getEventInfo(event.eventType);
                lastedNoReactionEvents[camera.id] = lastedNoReactionEvents[camera.id] || [];
                lastedNoReactionEvents[camera.id].unshift(event);

                event.eventTimeout ? $timeout.cancel(event.eventTimeout) : false;
                event.eventTimeout = $timeout(function() {
                    lastedNoReactionEvents[camera.id] = lastedNoReactionEvents[camera.id].filter(function(ev) {
                        return ev.id !== event.id;
                    });
                }, 300000);
                break;
    ...
    /* Стартуем мониторинг событий камер */
    var startMonitoring = function(camera) {
        MonitoringService.longPoolMonitoring(camera).then(
            function(response) {
                if (response !== false) {
                ...
                if (response) {
                    camera.lastTimeStamp = response.ts + 1;
                } else {
                    if (response === false) {
                        camera.defer = false;
                        analiseMonitoringEvents();
                    }
                    return
                }
                response.data.map(function(event) {
                    addEventForCamera(event, camera);
                });
                analiseMonitoringEvents();
            },
            function() {
    ...
    $scope.$watch('currentQuadrator.cameras', function(newQuadrator, oldQuadrator) {
        if (!newQuadrator) return;


        $scope.usedCamerasOfQuadrator = [];
        oldQuadrator ? oldQuadrator.map(function(cam) {
            cam.defer ? cam.defer.resolve(false) : false;
        }) : false;

        var statParams = JSON.parse(localStorage['statParams']) || {};
        statParams.camera = newQuadrator[0]['id'];

        localStorage['statParams'] = JSON.stringify(statParams);

        newQuadrator.map(function(item, index) {
            $scope.usedCamerasOfQuadrator.push(item.id);
    ...
    $scope.setCamera = function(newCamera, oldCamera) {
        var newQudratorCameras = angular.copy($scope.currentQuadrator.cameras);
        // newQudratorCameras[$scope.currentQuadrator.cameras.indexOf(oldCamera)] = newCamera;
        $scope.usedCamerasOfQuadrator[$scope.usedCamerasOfQuadrator.indexOf(oldCamera.id)] = newCamera.id;
        $scope.currentQuadrator.cameras[$scope.currentQuadrator.cameras.indexOf(oldCamera)] = angular.copy(newCamera);
    };
    ...
    $scope.quadratorsSelectParams = {
        'label': 'name',
        'class': 'cameras-select select-1'
    };

    $scope.headerQuadratorsSelectParams = angular.copy($scope.quadratorsSelectParams);
    ...
    var dateInterval = $interval(getNewTime, 400);

    var destroyPooling = function() {
        var cameras = $scope.currentQuadrator.cameras;
        $scope.currentQuadrator = false;
        $interval.cancel(dateInterval);
        cameras.map(function(cam) {
            $timeout.cancel(cam.longPoolTimer);
            cam.defer.resolve(false);
        });
    };
    $scope.$on('$destroy', destroyPooling);

    /* Управление наборами камер */
    /* Добавление квадратора в коллекцию квадраторов */
    var addQuadrator = function(params) {
        var collectionData = angular.copy(params);
        saveDialog(collectionData, function(val) {
            collectionData.name = val;
            collectionData.cameras = [];
            params.cameras.map(function(cam) {
                collectionData.cameras.push(cam.id);
            });
            CollectionsService.createCollection(collectionData).then(function(response) {
                collectionData.id = response.data.id;
                $scope.camerasCollection = collectionData;
                $scope.camerasCollectionList.push(collectionData);
                checkCamerasList();
            });
        });
    ...
    /* Переименование квадратора */
    var renameQuadrator = function() {
        saveDialog($scope.currentQuadrator, function(val) {
            $scope.camerasCollection.name = $scope.currentQuadrator.name = val;
            CollectionsService.createCollection($scope.currentQuadrator);
        });
    };

    /* Удаление квадратора */
    var deleteQuadrator = function() {
        CollectionsService.deleteCollection($scope.currentQuadrator.id).then(function() {
            $scope.camerasCollectionList = $scope.camerasCollectionList.filter(function(collection) {
                return collection.id !== $scope.currentQuadrator.id;
            });
            checkCamerasList();
        });
    };

    /* Сохранение квадратора */
    var saveQuadrator = function(asNew) {
        if (asNew) {
            var requestCollection = angular.copy($scope.currentQuadrator);
            requestCollection.id = undefined;
            addQuadrator(requestCollection);
        } else {
            $scope.camerasCollection.cameras = [];
            $scope.currentQuadrator.cameras.map(function(cam) {
                $scope.camerasCollection.cameras.push(cam.id);
            });
            $scope.camerasCollection.mode = $scope.currentQuadrator.mode;
            CollectionsService.createCollection($scope.camerasCollection);
        }
    };

    /* Опции выпадающего меню для редактирования квадраторов */
    var quadratorsEditSelect = [
        {
            label: 'Редактировать',
            disabled: true
        }, {
            label: 'Сохранить',
            action: saveQuadrator
        }, {
            label: 'Создать',
            action: function(){
                saveQuadrator(true);
            ...
            label: 'Переименовать',
            action: renameQuadrator
        }, {
            label: 'Удалить',
            action: deleteQuadrator
        }
    ];
    var quadratorsCreateSelect = [
        {
            label: 'Редактировать',
            disabled: true
        }, {
    ...
    /* Получаем параметры для дропдауна редактирования квадратора */
    $scope.getQuadratorsEditList = function() {
        if (!$scope.currentQuadrator) return quadratorsCreateSelect;
        return $scope.currentQuadrator.id ?
            quadratorsEditSelect :
            quadratorsCreateSelect;
    };

    /* Отслеживаем событие редактирования квадратора */
    $scope.$watch('quadratorsEditModel', function() {
        if (!$scope.quadratorsEditModel) return;
        if ($scope.quadratorsEditModel.action) {
            $scope.quadratorsEditModel.action();
        }
        $scope.quadratorsEditModel = $scope.getQuadratorsEditList()[0];
    });

    /* Выбор сохраненного набора камер */
    $scope.$watch('camerasCollection', function() {
        if ($scope.camerasCollection) {
            var quadrator = angular.copy($scope.camerasCollection);
            localStorage['camCollection'] = $scope.camerasCollection.id || undefined;
            if ($scope.camerasCollection.cameras) {
                quadrator.cameras = [];
                $scope.camerasCollection.cameras.map(function(cam, index) {
                    var currentCamera = camerasList.filter(function(camera) {
                        return camera.id === cam;
                    })[0];
                    if (currentCamera) {
                        quadrator.cameras.push(angular.copy(currentCamera));
                    }
                });
            }

            $scope.currentQuadrator = quadrator;
            if (!$scope.camerasCollection.cameras) {
                quadrator.cameras = [];
                $scope.setViewMode(3);
            }
    ...
    /* Управление окнами предпросмотра - zoom окна и просмотр последнего события */
    var lastedNoReactionEvents = {};
    var lastedEvents = {};

    /* Просмотр события */
    var iframeContainer = angular.element('<div>');
    var iframe = angular.element('<iframe>').addClass('video-repeater').appendTo(iframeContainer);
    var initedDialog = false;
    $scope.openEvent = function(event, camera) {
        lastedNoReactionEvents[camera.id] = lastedNoReactionEvents[camera.id].filter(function(ev) {
            if ((ev.id === event.id) || (ev.confidence >= $scope.confidence)) {
                $timeout.cancel(event.eventTimeout);
                return false;
            } else {
                return true;
            }
        ...
        iframe.attr('src', '/video.html?video=' + encodeURIComponent('http://' + parser.hostname +':8080/cam' + camera['id'] + '/alertFragments/alert' + event.id + '.mp4'));
        var dialogParams = {
            title: camera.group.name + ' - ' + camera.name,
            resizable: true
        };
    ...
    $scope.reactionDisabled = true;


    $scope.securityReaction = function(action, camera) {
        MonitoringService.securityReaction(camera.visibleEvent.id, action, camera)
            .then(function () {
                    // Ничего не делаем - ждём ответ Long pull запроса
                }, function () {
                    $scope.error = 'Нет соединения';
                }
            );
    };


    /* Отображение одной камеры квадратора */
    var oldShowedQuadrator = false;
    $scope.showCameraPreview = function(camera) {
        oldShowedQuadrator = angular.copy($scope.currentQuadrator);
        $scope.currentQuadrator.mode = 0;
        camera.stream = false;

        $scope.currentQuadrator.cameras = angular.copy([camera]);
        $scope.zoomedCamera = $scope.currentQuadrator.cameras[0];
    };
    $scope.hideCameraPreview = function() {
        $scope.currentQuadrator.mode = oldShowedQuadrator.mode;
        $scope.currentQuadrator.cameras = oldShowedQuadrator.cameras;
        $scope.zoomedCamera = false;
    };
...
var module = angular.module('Directives');
module.directive('ngVideoPlayer', function($timeout, $interval) {
    return {
        'restrict': 'A',
        'scope': {
            ngVideoPlayer: '='
        ...
        'link': function ($scope, element) {
            element.attr('id', $scope.ngVideoPlayer.id);

            var checkUrl = function() {
                if (!$scope.ngVideoPlayer.streamMode && ($scope.ngVideoPlayer.camera.analysis !== 1)) {
                    return $scope.ngVideoPlayer.mode <= 2 ? $scope.ngVideoPlayer.camera.output_url : $scope.ngVideoPlayer.camera.output_vascaled_url;
                } else {
                    return $scope.ngVideoPlayer.camera.output_vasrc_url;
                }
            };

            var createPlayer = function() {
                hdwplayer({
                    id: $scope.ngVideoPlayer.id,
                    swf: '/static/media/swf/player.swf',
                    type: 'rtmp',
                    streamer: checkUrl(),
                    video: 'cam' + $scope.ngVideoPlayer.camera.id,
                    autoStart: 'true',
                    volumeLevel: 0,
                    controlBar: 'false',
                    width   : '640',
                ...
                cameraTimer = $timeout(createPlayer, 600000);
            };

            var cameraTimer = $timeout(createPlayer, 500 * $scope.ngVideoPlayer.index);

            if ($scope.ngVideoPlayer.camera.analysis !== 1) {
                $scope.$watch('ngVideoPlayer.streamMode', function() {
                    $timeout.cancel(cameraTimer);
                    createPlayer();
                });
            }
            ...
            $scope.$watch('ngVideoPlayer', function() {
                $timeout.cancel(cameraTimer);
                createPlayer();
            });
        }
    ...
    $stateProvider.state('main', {
        ...
        resolve: {
            ...
            userCameras: function(CamerasService, $q, currentUser, $state) {
                ...
                var defer = $q.defer();
                CamerasService.getUserCamerasList().then(function(response) {
                    defer.resolve(response);
                }, function() {
                    defer.resolve(false);
                });
        ...
        resolve: {
            camerasCollections: function(CollectionsService) {
                return CollectionsService.getList();
            }
        }
...
angular
    .module('Services')
    .service('RecordsService', function($q, $http, $timeout) {
        return {
            getStatistics: function(startDate, endDate, startTime, endTime, camera) {
                var deffered = $q.defer();
                $http.get(camera.events_url + 'db1.json?startDate='+startDate+'&endDate='+endDate+'&startTime='+startTime+'&endTime='+endTime +'&events=1')
                    .then(function(result) {
